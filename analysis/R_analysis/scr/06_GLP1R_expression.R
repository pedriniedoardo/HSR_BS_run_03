...
df_tot %>%
  ...
  #       panel.border = element_rect(colour = "black", fill = NA))
  theme(strip.background = element_blank())
ggsave("../../out/image/06_UMAPggplot_annotationConfident_GLP1R_count2.pdf",width = 6,height = 5)

# do the same using Seurat
FeaturePlot(data.combined,features = GOI,raster = T,order = T)
ggsave("../../out/image/06_UMAPSeurat_annotationConfident_GLP1R_count.pdf",width = 6,height = 5)
...
# save the table
df_avg2 %>%
  write_tsv("../../out/table/06_df_avg_cellTypeTreat_GLP1R_BS.tsv")
