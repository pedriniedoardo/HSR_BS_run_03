...
df_tot %>%
  ...
  theme(strip.background = element_blank())
ggsave("../../out/image/UMAPggplot_annotationConfident_BTK_count.pdf",width = 13,height = 12)

# do the same using Seurat
FeaturePlot(data.combined,features = GOI,split.by = "treat_full",raster = T,order = T)
ggsave("../../out/image/UMAPSeurat_annotationConfident_BTK_count.pdf",width = 25,height = 3)
...
df_tot %>%
  ...
  #       panel.border = element_rect(colour = "black", fill = NA))
  theme(strip.background = element_blank())
ggsave("../../out/image/UMAPggplot_annotationConfident_BTK_count2.pdf",width = 13,height = 12)
...
df_tot %>%
  ...
  #       panel.border = element_rect(colour = "black", fill = NA))
  theme(strip.background = element_blank())
ggsave("../../out/image/UMAPggplot_annotationConfident_BTK_minmax.pdf",width = 13,height = 12)
...
df_tot %>%
  ...
  #       panel.border = element_rect(colour = "black", fill = NA))
  theme(strip.background = element_blank())
ggsave("../../out/image/UMAPggplot_annotationConfident_BTK_proppos.pdf",width = 13,height = 12)
...
df_tot %>%
  ...
  theme(strip.background = element_blank(),
        panel.border = element_rect(colour = "black", fill = NA))
ggsave("../../out/image/violin_annotationConfident_BTK.pdf",width = 15,height = 10)
...
ggsave("../../out/image/dotplot_annotationConfident_BTK_expressionAvg_treatFull.pdf",width = 9,height = 9)
...
df_avg |>
  ...
  facet_wrap(~expertAnno.l1,scales = "free")
# scale_fill_viridis_c(option = "plasma",name="log10 number \nof cells")
ggsave("../../out/image/dotplot_annotationConfident_BTK_expressionAvg_treat.pdf",width = 9,height = 9)
...
df_avg |>
  ...
  facet_wrap(~expertAnno.l1)
# scale_fill_viridis_c(option = "plasma",name="log10 number \nof cells")
ggsave("../../out/image/dotplot_annotationConfident_BTK_expressionAvg_treat_scale.pdf",width = 9,height = 9)
...
# save the table
df_avg2 |> 
  write_tsv("../../out/table/df_avg_cellTypeTreat_BTK_BS.tsv")
...
df_avg2 |>
  ...
  facet_wrap(~expertAnno.l1,scales = "free")
# scale_fill_viridis_c(option = "plasma",name="log10 number \nof cells")
ggsave("../../out/image/dotplot_annotationConfident_BTK_expressionAvg_treatFull2.pdf",width = 9,height = 9)
...
# 
# df_test_subset |> 
#   write_tsv("../../out/table/FindAllMarkers_cellID_treat_minpct5_logfcthr01.tsv")

df_test_subset <- read_tsv("../../out/table/FindAllMarkers_cellID_treat_minpct5_logfcthr01.tsv")

# check if BTK is in there
df_test_subset |> 
  filter(gene %in% GOI) |> 
  ggplot(aes(x = avg_log2FC,y = -log(p_val_adj),col=cluster,label=cluster))+geom_point()+facet_wrap(~cell_id,scales = "free")+theme_bw()+theme(strip.background = element_blank())+
...
# ggsave("../../out/image/DE_annotationConfident_TSPO_sc.pdf",width = 12,height = 9)
...
ggsave("../../out/image/DE_annotationConfident_BTK_bulk_PairWiseTest.pdf",width = 9,height = 9)
...
ggsave("../../out/image/DE_annotationConfident_BTK_bulk_PairWiseTest2.pdf",width = 9,height = 9)
...
df_avg_cellType |>
  ...
  theme(strip.background = element_blank(),
        panel.border = element_rect(colour = "black", fill = NA))
# scale_fill_viridis_c(option = "plasma",name="log10 number \nof cells")
ggsave("../../out/image/dotplot_annotationConfident_BTK_expressionAvg_cellType.pdf",width = 6,height = 5)
...
df_avg_cellType2 |>
  ...
  theme(strip.background = element_blank(),
        panel.border = element_rect(colour = "black", fill = NA))+scale_color_manual(values = c("black","red"))
# scale_fill_viridis_c(option = "plasma",name="log10 number \nof cells")
ggsave("../../out/image/dotplot_annotationConfident_BTK_expressionAvg_cellType2.pdf",width = 6,height = 5)

# save the table
df_avg_cellType2 |> 
  write_tsv("../../out/table/df_avg_cellType_BTK_BS.tsv")
...
#   write_tsv("../../out/table/FindAllMarkers_annotation_confident_PosOnly_minpct5_logfcthr01.tsv")

sobj_total_h.markers <- read_tsv("../../out/table/FindAllMarkers_annotation_confident_PosOnly_minpct5_logfcthr01.tsv")
sobj_total_h.markers |> 
  filter(gene %in% GOI)
...
# # save the table
# df_avg_disease2 |> 
#   write_tsv("../../out/table/df_avg_disease_TSPO_BS.tsv")
...
df_avg_disease |>
  ...
  theme(strip.background = element_blank(),
        panel.border = element_rect(colour = "black", fill = NA))
# scale_fill_viridis_c(option = "plasma",name="log10 number \nof cells")
ggsave("../../out/image/dotplot_annotationConfident_BTK_expressionAvg_disease.pdf",width = 6,height = 5)
...
#       panel.border = element_rect(colour = "black", fill = NA))
# scale_fill_viridis_c(option = "plasma",name="log10 number \nof cells")
ggsave("../../out/image/dotplot_annotationConfident_BTK_expressionAvg_disease2.pdf",width = 6,height = 5)
...
# test pos BTK vs neg BTK -------------------------------------------------
# subset only the MG cells. define the positive and negative cells per BTK and run a de analysis to compare pull the genes
...
sobj_MG_BTK <- FetchData(sobj_MG,"BTK",slot = "data") |>
  rownames_to_column("barcodes")

# add the TSPO expression to the original meta
sobj_MG$BTK <- sobj_MG@meta.data |>
  rownames_to_column("barcodes") |>
  left_join(sobj_MG_BTK) |>
  pull("BTK")

# build a category form the expression value
sobj_MG$BTK_cat <- case_when(sobj_MG$BTK == 0~"neg",
                              T~"pos")

# summarise the number of cells
table(sobj_MG$BTK_cat)

# ggplot(aes(x=BTK))+geom_histogram()

# run the DE
Idents(sobj_MG) <- "BTK_cat"

# avg_logFC: log fold-chage of the average expression between the two groups. Positive values indicate that the gene is more highly expressed in the first group
res_test_BTK_MG <- RunPresto(object = sobj_MG,ident.1 = "pos",ident.2 = "neg")

# save the table of top markers
res_test_BTK_MG |>
  rownames_to_column("gene") |>
  mutate(cell_id = "MG") |>
  write_tsv("../../out/table/res_test_BTK_MG.tsv")

# plot volcano
volcano_tot <- read_tsv("../../out/table/res_test_BTK_MG.tsv") %>%
...
ggsave("../../out/image/volcano_test_BTK_MG.pdf",width = 12,height = 9)

# explore the expression only inside the positive cells -------------------
# from the MG cell extract only the positive cells
sobj_MG_BTKpos <- subset(sobj_MG,subset = BTK_cat == 'pos')

sobj_MG_BTKpos@meta.data

# run the DE
Idents(sobj_MG_BTKpos) <- "treat_full"

# how many cells are in the different categories
table(Idents(sobj_MG_BTKpos))

# avg_logFC: log fold-chage of the average expression between the two groups. Positive values indicate that the gene is more highly expressed in the first group
# vs baseline
df_res_BTKpos <- lapply(c("myelin","CSF.MS.24h","CSF.ctrl.24h","cytokine","CSF.MS.48h","TBHP","Fe"),function(x){
  # track the progress
  print(x)
  res_MG_BTKpos <- RunPresto(object = sobj_MG_BTKpos,ident.1 = x,ident.2 = "BASELINE", logfc.threshold = 0.1) %>%
    rownames_to_column("gene") %>%
    mutate(comparison = paste0(x,"_vs_BASELINE"))
  return(res_MG_BTKpos)
}) %>%
  bind_rows()

# extract the expression of BTK
df_res_BTKpos %>%
  filter(gene %in% c("BTK"))

# vs CSF.ctrl.24h
df_res_BTKpos2 <- lapply(c("myelin","CSF.MS.24h","BASELINE","cytokine","CSF.MS.48h","TBHP","Fe"),function(x){
  # track the progress
  print(x)
  res_MG_BTKpos <- RunPresto(object = sobj_MG_BTKpos,ident.1 = x,ident.2 = "CSF.ctrl.24h",logfc.threshold = 0.1) %>%
    rownames_to_column("gene") %>%
