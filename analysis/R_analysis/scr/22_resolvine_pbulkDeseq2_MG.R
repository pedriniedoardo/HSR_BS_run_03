...
scobj_subset <- subset(data.combined,subset = expertAnno.l1 %in% c("MG") & harmonized_donor2 %in% c("donRR16","donRR24","donRR25"))
...
saveRDS(ddsHTSeq_filter,"../../out/object/22_ddsHTSeq_MG_pseudobulk_filterExp.rds")
...
contrast <- makeContrasts(MG_Cytokine_vs_BASELINE = treatcytokine,
                          MG_CSFCtrl24_vs_BASELINE = treatCSF.ctrl.24h,
                          MG_CSFMS24_vs_BASELINE = treatCSF.MS.24h,
                          MG_CSFMS48_vs_BASELINE = treatCSF.MS.48h,
                          MG_Fe_vs_BASELINE = treatFe,
                          MG_Myelin_vs_BASELINE = treatmyelin,
                          MG_TBHP_vs_BASELINE = treatTBHP,
                          levels = design)

# pull the results table
res_cytokine <- results(ddsHTSeq_filter, contrast=contrast[,"MG_Cytokine_vs_BASELINE"],alpha = 0.05)
res_CSFCtrl24 <- results(ddsHTSeq_filter, contrast=contrast[,"MG_CSFCtrl24_vs_BASELINE"],alpha = 0.05)
res_CSFMS24 <- results(ddsHTSeq_filter, contrast=contrast[,"MG_CSFMS24_vs_BASELINE"],alpha = 0.05)
res_CSFMS48 <- results(ddsHTSeq_filter, contrast=contrast[,"MG_CSFMS48_vs_BASELINE"],alpha = 0.05)
res_Fe <- results(ddsHTSeq_filter, contrast=contrast[,"MG_Fe_vs_BASELINE"],alpha = 0.05)
res_Myelin <- results(ddsHTSeq_filter, contrast=contrast[,"MG_Myelin_vs_BASELINE"],alpha = 0.05)
res_TBHP <- results(ddsHTSeq_filter, contrast=contrast[,"MG_TBHP_vs_BASELINE"],alpha = 0.05)
...
# save the table
df_res %>%
  write_tsv("../../out/table/22_DE_pseudobulk_MG_filterExp.tsv")
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/22_DE_pseudobulk_MG_filterExp_shr.tsv")
...
ggsave("../../out/image/22_histogram_pvalue_pseudobulk_MG_filterExp.pdf",width = 6,height = 4)
...
ggsave("../../out/image/22_vulcano_plot_pseudobulk_MG_filterExp.pdf",width = 10,height = 10)
...
plot_volcano_shr %>%
  ...
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
ggsave("../../out/plot/22_vulcano_plot_pseudobulk_MG_filterExp_shr.pdf",width = 10,height = 10)
...
pdf("../../out/image/22_heatmap_DEG_plot_pseudobulk_MG_filterExp_shr.pdf",width = 7,height = 8)
...
pdf("../../out/image/22_heatmap_DEG_plot_pseudobulk_MG_cytokinesALL_filterExp_shr.pdf",width = 5,height = 8)
...
pdf("../../out/image/22_heatmap_DEG_plot_pseudobulk_MG_cytokinesSIG_filterExp_shr.pdf",width = 5,height = 7)
...
pdf("../../out/image/22_heatmap_DEG_plot_pseudobulk_MG_csfmsALL_filterExp_shr.pdf",width = 5,height = 9)
...
pdf("../../out/image/22_heatmap_DEG_plot_pseudobulk_MG_csfmsSIG_filterExp_shr.pdf",width = 5,height = 7)
...
ggsave("../../out/image/22_boxplot_MG_GOI.pdf",width = 20,height = 20)
...
# PLOT DISPERSION ---------------------------------------------------------
pdf("../../out/image/22_dispersion_plot_pseudobulk_MG_filterExp.pdf",width = 5,height = 5)
