...
list_cell_embeddings <- split(full_data,f = full_data$treat_full) %>%
  lapply(function(x){
    x %>%
      dplyr::select("UMAP_1","UMAP_2")
  })

list_vars <- split(full_data,f = full_data$treat_full) %>%
  lapply(function(x){
    x %>%
      # dplyr::select("CD4")
...
df_density %>%
  ...
  labs(color = "density") +
  theme_void() +
  theme(strip.background = element_blank()) + facet_wrap(~treat_full)+scale_color_viridis_c(option = "turbo")+
