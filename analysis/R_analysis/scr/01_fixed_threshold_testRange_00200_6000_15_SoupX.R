...
# READ IN DATA ------------------------------------------------------------
# define the range of test for the thresholds
featureLow_thr <- 200
featureHigh_thr <- 6000
mito_thr <- 15
label <- "00200_06000_15"
...
# c("orig.ident","nCount_RNA","nFeature_RNA","percent.mt","percent.ribo","mt_bin","treat","clone","doxy","exposure","ID","test","not_outlier","RNA_snn_res.0.8","seurat_clusters","pANN","DF")
...
list_norm_doublet <- pmap(list(names(list_datasc_fixed_norm_doublet),list_datasc_fixed_norm_doublet),function(x,y){
  ...
  meta <- y@meta.data
  colnames(meta) <- c("orig.ident","nCount_RNA","nFeature_RNA","percent.mt","percent.ribo","mt_bin","treat","clone","doxy","exposure","ID","test","not_outlier","RNA_snn_res.0.8","seurat_clusters","pANN","DF")
  y@meta.data <- meta
  # head(y@meta.data)
  return(y)
}) %>% 
