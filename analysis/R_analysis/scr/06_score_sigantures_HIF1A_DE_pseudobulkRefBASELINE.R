...
# 1. Get counts matrix
# focus only on BASELINE, Fe and Cytokine treatment
counts_MG <- cts_sample_all$RNA %>% 
  data.frame() %>% 
  rownames_to_column("gene") %>% 
  # pull only the MG cells
  dplyr::select("gene",contains("MG")) %>% 
  # pull only the condition of interest
  dplyr::select("gene",contains("BASELINE")|contains("Fe")|contains("cytokine")) %>% 
...
colData_MG <- data.frame(samples = colnames(counts_MG)) %>%
  ...
  mutate(treat_full = factor(treat_full,levels = c("BASELINE","cytokine","Fe")))
...
# save the disign
# saveRDS(design_MG,"../../out/object/design_MG_refBASELINE.rds")
...
# filter
# filter based on the ovrall sample distribution
keep_MG <- edgeR::filterByExpr(counts(dds_MG), group = colData_MG$treat_full)
...
# pdf("../../out/image/heatmap_SampleCluster_pseudobulk_MG_refBASELINE.pdf",width = 12,height = 6)
# set.seed(1)
...
# ggsave("../../out/image/PCA_pseudobulk_MG_refBASELINE.pdf",width = 6,height = 4)
...
# saveRDS(ddsHTSeq_MG_filter,"../../out/object/ddsHTSeq_pseudobulk_MG_refBASELINE.rds")
...
# contrast_MG <- makeContrasts(CSF.ctrl.24h_vs_BASELINE = treat_fullCSF.ctrl.24h,
#                              CSF.MS.24h_vs_BASELINE = treat_fullCSF.MS.24h,
#                              CSF.MS.48h_vs_BASELINE = treat_fullCSF.MS.48h,
#                              cytokine_vs_BASELINE = treat_fullcytokine,
#                              Fe_vs_BASELINE = treat_fullFe,
#                              myelin_vs_BASELINE = treat_fullmyelin,
#                              TBHP_vs_BASELINE = treat_fullTBHP,
#                              levels = design_MG)
contrast_MG <- makeContrasts(cytokine_vs_BASELINE = treat_fullcytokine,
                             Fe_vs_BASELINE = treat_fullFe,
                             levels = design_MG)

# pull the results table
# res_CSF.ctrl.24h <- results(ddsHTSeq_MG_filter, contrast=contrast_MG[,"CSF.ctrl.24h_vs_BASELINE"],alpha = 0.05)
# summary(res_CSF.ctrl.24h)
# res_CSF.MS.24h <- results(ddsHTSeq_MG_filter, contrast=contrast_MG[,"CSF.MS.24h_vs_BASELINE"],alpha = 0.05)
# summary(res_CSF.MS.24h)
# res_CSF.MS.48h <- results(ddsHTSeq_MG_filter, contrast=contrast_MG[,"CSF.MS.48h_vs_BASELINE"],alpha = 0.05)
# summary(res_CSF.MS.48h)
res_cytokine <- results(ddsHTSeq_MG_filter, contrast=contrast_MG[,"cytokine_vs_BASELINE"],alpha = 0.05)
summary(res_cytokine)
res_Fe <- results(ddsHTSeq_MG_filter, contrast=contrast_MG[,"Fe_vs_BASELINE"],alpha = 0.05)
summary(res_Fe)
# res_myelin <- results(ddsHTSeq_MG_filter, contrast=contrast_MG[,"myelin_vs_BASELINE"],alpha = 0.05)
# summary(res_myelin)
# res_TBHP <- results(ddsHTSeq_MG_filter, contrast=contrast_MG[,"TBHP_vs_BASELINE"],alpha = 0.05)
# summary(res_TBHP)

# add the gene symbols
df_res <- 
  list(# CSF.ctrl.24h = res_CSF.ctrl.24h,
...
# save the table
# df_res %>% write_tsv("../../out/table/DE_treatvsBASELINE_pseudobulk_MG.tsv")

# shrink ------------------------------------------------------------------  
# res_CSF.ctrl.24h_shr <- lfcShrink(ddsHTSeq_MG_filter, res = res_CSF.ctrl.24h, type = "ashr")
...
df_res_shr <- 
  list(# CSF.ctrl.24h_shr = res_CSF.ctrl.24h_shr,
  ...
  bind_rows(.id = "conditionVsBASELINE")
# save the table
# df_res_shr %>% write_tsv("../../out/table/DE_treatvsBASELINE_pseudobulk_MG_shr.tsv")
...
# ggsave("../../out/image/histogram_pvalue_pseudobulk_MG_refBASELINE.pdf",width = 6,height = 6)
...
plot_volcano %>%
  ...
  ggrepel::geom_text_repel(
    data = plot_volcano %>% group_by(conditionVsBASELINE) %>% arrange(padj) %>% dplyr::slice(1:10),
    aes(label = symbol),segment.alpha=0.4) +
  facet_wrap(~conditionVsBASELINE)+
  theme_bw()+
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
# ggsave("../../out/image/vulcano_plot_pseudobulk_MG_refBASELINE.pdf",width = 20,height = 20)
...
plot_volcano_shr %>%
  ...
  scale_color_manual(values = c("black","red"))+theme(legend.position = "none")+
  ggrepel::geom_text_repel(
    data = plot_volcano_shr %>% group_by(conditionVsBASELINE) %>% arrange(padj) %>% dplyr::slice(1:10),
    aes(label = symbol),segment.alpha=0.4) +
  facet_wrap(~conditionVsBASELINE)+
  theme_bw()+
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
# ggsave("../../out/image/vulcano_plot_pseudobulk_MG_refBASELINE_shr.pdf",width = 20,height = 20)
...
df_res %>%
  ...
  theme(strip.background = element_blank()) +
  theme(legend.position = "none")
# ggsave("../../out/image/MA_plot_pseudobulk_MG_refBASELINE.pdf",width = 20,height = 20)
...
df_res_shr %>%
  ...
  theme(strip.background = element_blank()) +
  theme(legend.position = "none")
# ggsave("../../out/image/MA_plot_pseudobulk_MG_refBASELINE_shr.pdf",width = 20,height = 20)

# heatmaps ----------------------------------------------------------------
# among all the genes, focus on the one belonging to the response to hipoxia siganture
msigdbr_collections() %>%
  print(n=30)
...
# filter among all the comparison only the genes form the signature that can be labelled as significantly altered in the comparison with the BASELINE sample
# the DEGs plot
DEG_2 <- df_res_shr %>%
  data.frame() %>%
  # filter the one in the signature
  filter(symbol %in% pathways$REACTOME_CELLULAR_RESPONSE_TO_HYPOXIA) %>%
  # add a clor variable in case significant
...
# pdf("../../out/image/heatmap_DEG_plot_pseudobulk_MG_refBASELINE_shr.pdf",width = 7,height = 14) 
draw(ht2_shr_MG,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm")) 
# dev.off()

# # upset plot --------------------------------------------------------------
# # read in the table of DEGs
# df_res_shr <- read_tsv("../../out/table/DE_treatvsBASELINE_pseudobulk_MG_shr.tsv")
# 
...
# 
# # PLOT DISPERSION ---------------------------------------------------------
# pdf("../../out/image/dispersion_plot_pseudobulk_MG_refBASELINE.pdf",width = 5,height = 5) 
