...
list_test <- lapply(names(table(data.combined$expertAnno.l1)),function(cell_id){
  ...
  # run the test using as reference BASELINE vs all the other treatments one by one
  # loop the testing
  list_DE <- lapply(c("myelin","CSF.MS.24h","CSF.ctrl.24h","cytokine","CSF.MS.48h","TBHP","Fe"),function(condition){
    print(paste(cell_id,condition))
    test <- RunPresto(test_obj, ident.1 = condition,ident.2 = "BASELINE",min.pct = 0.05, logfc.threshold = 0,return.thresh=1,only.pos = F) %>%
...
df_test_subset |> 
  write_tsv("../../out/table/DE_treatvsBASELINE_cellIDwise_minpct5.tsv")

# plot volcano ------------------------------------------------------------
# plot the volcano for the cluster 12 DE
volcano_tot <- read_tsv("../../out/table/DE_treatvsBASELINE_cellIDwise_minpct5.tsv") %>%
...
ggsave("../../out/image/volcano_DE_treatvsBASELINE_cellIDwise_minpct5.pdf",width = 25,height = 20)
...
ggplot() +
  ...
  facet_grid(test~cell_id)+
  theme(strip.background = element_blank())
ggsave("../../out/image/volcano_DE_treatvsBASELINE_cellIDwise_minpct5_noLab.pdf",width = 25,height = 20)
...
# save the tables
list_enrichr_UP %>%
  write_tsv("../../out/table/enrichR_DE_treatvsBASELINE_cellIDwise_minpct5_UP.tsv")

list_enrichr_DOWN %>%
  write_tsv("../../out/table/enrichR_DE_treatvsBASELINE_cellIDwise_minpct5_DOWN.tsv")
...
ggsave("../../out/image/enrichR_DE_treatvsBASELINE_cellIDwise_minpct5_UP.pdf",width = 40,height = 60,limitsize = FALSE)
...
ggsave("../../out/image/enrichR_DE_treatvsBASELINE_cellIDwise_minpct5_DOWN.pdf",width = 40,height = 60,limitsize = FALSE)

# confirm some of the trends
list_enrichr_UP |> 
  filter(comparison == "MG|CSF.MS.48h_BASELINE|UP") |> 
  head()
