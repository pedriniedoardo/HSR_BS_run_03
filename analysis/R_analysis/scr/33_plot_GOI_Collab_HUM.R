...
ggsave("../../out/image/33_Histogram_annotationConfident_collabHUM_count.pdf",width = 6,height = 5)
...
# FeaturePlot(data.combined,features = GOI,raster = F,order = T)
ggsave("../../out/image/33_UMAPSeurat_annotationConfident_collabHUM_count.pdf",width = 6,height = 5)
...
ggsave(plot = p01,filename = "../../out/image/33_UMAPSeuratNebulosa_annotationConfident_collabHUM_count.pdf",width = 12,height = 10)
...
# save the table
df_avg %>%
  write_tsv("../../out/table/33_df_avg_cellTypeTreat_collabHUM_BS03.tsv")
...
ggsave("../../out/image/33_avgExp_cellid_annotationConfident_collabHUM.pdf",width = 6,height = 5)

# plot the average expresison by condition
df_avg |>
  # ggplot(aes(x=NMDA_time,y=count)) + 
  ...
  theme(strip.background = element_blank(),
        panel.border = element_rect(colour = "black", fill = NA))+
  facet_wrap(~gene,scales = "free")
ggsave("../../out/image/33_avgExp_treat_annotationConfident_collabHUM.pdf",width = 6,height = 5)
...
ggsave("../../out/image/33_avgExpSplit_annotationConfident_collabHUM.pdf",width = 12,height = 5)
...
# scale_fill_viridis_c(option = "plasma",name="log10 number \nof cells")
ggsave("../../out/image/33_avgExp_donor_annotationConfident_collabHUM.pdf",width = 9,height = 5)
...
ggsave("../../out/image/33_DEvsAll_collabHUM_sc.pdf",width = 20,height = 6)
...
ggsave("../../out/image/33_DEtreat_minpct5_logfcthr01_refBASELINE_collabHUM.pdf",width = 17,height = 15)

# position of GOI in the overall gene expression distribution ---------------
# mean log-normalised expression per gene across all cells
avg_all_genes <- rowMeans(GetAssayData(data.combined, layer = "data"))

df_gene_dist <- tibble(
  gene = names(avg_all_genes),
  avg_exp = avg_all_genes
) %>%
  mutate(percentile = rank(avg_exp) / n() * 100,
         is_GOI = gene %in% GOI)

# print percentile rank for each GOI
df_gene_dist %>%
  filter(is_GOI) %>%
  select(gene, avg_exp, percentile) %>%
  print()

df_goi_label <- df_gene_dist %>% filter(is_GOI)

# overall distribution with GOI marked
df_gene_dist %>%
  ggplot(aes(x = avg_exp)) +
  geom_histogram(bins = 100, fill = "gray70", color = NA) +
  geom_vline(data = df_goi_label,
             aes(xintercept = avg_exp),
             col = "red", linetype = "dashed", linewidth = 0.9) +
  geom_text(data = df_goi_label,
            aes(x     = avg_exp,
                y     = Inf,
                label = sprintf("%s\n%.1f%%ile", gene, percentile)),
            vjust = 1.4, hjust = -0.15, color = "red", size = 3) +
  scale_x_log10() +
  theme_bw() +
  theme(strip.background = element_blank()) +
  labs(x = "Mean log-normalised expression across all cells (log10)",
       y = "Number of genes")
ggsave("../../out/image/33_geneDistribution_collabHUM_GOI_overall.pdf", width = 7, height = 5)
