...
ggsave(plot = p01,filename = "../../out/image/20_UMAPSeuratNebulosa_annotationConfident_set01_count.pdf",width = 12,height = 10)

# Valerio asked to add ALOX12 to the plot
# the issue is that the gene is not expressed at all from the dataset, thereofore we need to produce an ad-hoc dataset
# 1. Copy the first plot to use as a template
fake_panel <- p01[[1]]

# 2. Force the data being mapped to color to be 0
# In ggplot objects generated by Nebulosa, the density value is in the 'feature' column
fake_panel$data[[names(fake_panel$data)[grep("^feature", names(fake_panel$data))]]] <- 0

# 3. Update the title and the color scale to look "empty"
fake_panel <- fake_panel + 
  ggtitle("ALOX12") + 
  scale_color_viridis_c(limits = c(0, 0.2),) # Match your other plots' scale

p012 <- p01 + fake_panel + plot_layout(ncol = 3)
ggsave(plot = p012,filename = "../../out/image/20_UMAPSeuratNebulosa_annotationConfident_set01_count_custom.pdf",width = 12,height = 10)
...
# save the table
df_avg %>%
  write_tsv("../../out/table/20_df_avg_cellTypeTreat_resolvineSet01_BS03.tsv")
...
# Valerio asked to use the color of the cell id in the UMAP
# show_col(hue_pal()(7))
...
df_avg |>
  ...
  theme(strip.background = element_blank(),
        panel.border = element_rect(colour = "black", fill = NA))+
  facet_wrap(~gene,scales = "free")
ggsave("../../out/image/20_avgExp_annotationConfident_set01_custom.pdf",width = 12,height = 10)
...
# scale_fill_viridis_c(option = "plasma",name="log10 number \nof cells")
# ggsave("../../out/image/06_dotplot_annotationConfident_GLP1R_expressionAvg_treatFull.pdf",width = 9,height = 9)
ggsave("../../out/image/20_avgExpSplit_annotationConfident_set01_02_custom.pdf",width = 12,height = 10)
...
ggsave("../../out/image/20_FindMarkers_cellID_treat_minpct5_logfcthr01_refBASELINE.pdf",width = 17,height = 15)
