...
list_genes_UP <- read_tsv("../../out/table/DE_treatvsBASELINE_pseudobulk_MG_shr.tsv") %>%
...
list_genes_DOWN <- read_tsv("../../out/table/DE_treatvsBASELINE_pseudobulk_MG_shr.tsv") %>%
  filter(padj<0.05 & log2FoldChange < (-1)&!is.na(symbol)) %>%
  split(f = .$conditionVsBASELINE) %>%
  map(function(x){
    x %>%
...
list_enrichr_UP %>%
  write_tsv("../../out/table/enrichR_DE_treatvsBASELINE_pseudobulk_MG_UP_shr.tsv")
...
list_enrichr_DOWN %>%
  write_tsv("../../out/table/enrichR_DE_treatvsBASELINE_pseudobulk_MG_DOWN_shr.tsv")
...
ggsave("../../out/image/enrichR_DE_treatvsBASELINE_pseudobulk_MG_UP_shr.pdf",width = 40,height = 12,limitsize = FALSE)
...
ggsave("../../out/image/enrichR_DE_treatvsBASELINE_pseudobulk_MG_DOWN_shr.pdf",width = 40,height = 12,limitsize = FALSE)

# -------------------------------------------------------------------------
read_tsv("../../out/table/DE_treatvsBASELINE_pseudobulk_MG_shr.tsv") |> 
