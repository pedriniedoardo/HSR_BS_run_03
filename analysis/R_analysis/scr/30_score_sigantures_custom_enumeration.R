...
# read in the data --------------------------------------------------------
folder <-  "../../out/table/modules_custom/"
...
# save the table
df_en_090_final %>% 
  write_tsv("../../out/table/modules_custom/30_enumeration_ALL_090_threshold_custom.tsv")
...
ggsave(paste0("../../out/image/modules_custom/30_enumeration_log2Prop_ALL_090_threshold_custom.pdf"),width = 15,height =5)
...
# save the table
enumeration_cellID %>% 
  write_tsv("../../out/table/modules_custom/30_enumeration_ALL_090_threshold_sampleWise_custom.tsv")
...
ggsave("../../out/image/modules_custom/30_enumeration_log2Prop_ALL_090_threshold_above5_sampleWise_custom.pdf",width = 25,height =5)
...
enumeration_cellID %>%
  ...
  theme(strip.background = element_blank(),
        strip.text.y.right = element_text(angle = 0))+
  geom_hline(yintercept = 0.1,linetype="dotted",col="black")+theme(axis.text.x = element_text(hjust = 1,angle = 90))
# ggsave("../../out/image/modules_custom//30_enumeration_log2Prop_ALL_090_threshold_above5_sampleWise_small2_SENESCENCE.pdf",width = 25,height =30)
...
enumeration_cellID_zeroes %>% 
  write_tsv("../../out/table/modules_custom/30_enumeration_ALL_090_threshold_sampleWise_custom_zeroes.tsv")
...
enumeration_cellID_zeroes %>%
  ...
  theme(strip.background = element_blank(),
        strip.text.y.right = element_text(angle = 0))+
  geom_hline(yintercept = 0.1,linetype="dotted",col="black")+theme(axis.text.x = element_text(hjust = 1,angle = 90))
ggsave("../../out/image/modules_custom/30_enumeration_log2Prop_ALL_090_threshold_above5_sampleWise_custom_zeroes.pdf",width = 25,height =5)

# -------------------------------------------------------------------------
p1 <- enumeration_cellID_zeroes %>%
  filter(signature %in% c("ferroptosis_all")) %>%
  # filter(n_sen == 0)
  ggplot(aes(x=treat_full,y=prop_sen)) +
  geom_boxplot(outlier.shape = NA)+
  geom_point(position = position_jitter(width = 0.1),shape=1) +
