...
GOI <- c("ABCC1", "AKR1C3", "ALOX12", "ALOX15", "ALOX5", "CBR1", "CYP1A2", "CYP2C8", "CYP2C9", "CYP2D6", "CYP2E1", "CYP3A4", "CYP4F2", "CYP4F3", "DPEP1", "DPEP2", "DPEP3", "EPHX1", "EPHX2", "EPHX3", "GGT1", "GGT2", "GGT5", "GPX2", "GPX4", "GSTM4", "HPGDS", "LTA4H", "LTC4S", "PTGDS", "PTGES", "PTGES2", "PTGIS", "PTGS1", "PTGS2", "TBXAS1", "TXN")
...
ggsave("../../out/image/20_UMAPSeurat_annotationConfident_set02_count.pdf",width = 14,height = 24)

# try plotting gene expression with Nebulosa
GOI_subset <- str_subset(string = GOI,pattern = "ALOX12|CYP1A2|CYP2C8|CYP2C9|CYP4F2|DPEP1|DPEP3|GGT2",negate = T)
p01 <- plot_density(data.combined,features = GOI_subset,reduction = "umap")
ggsave(plot = p01,filename = "../../out/image/20_UMAPSeuratNebulosa_annotationConfident_set02_count.pdf",width = 18,height = 15)
...
# save the table
df_avg %>%
  write_tsv("../../out/table/20_df_avg_cellTypeTreat_resolvineSet02_BS03.tsv")
...
ggsave("../../out/image/20_avgExpSplit_annotationConfident_set02.pdf",width = 12,height = 40)
...
df_avg_summary <- df_avg %>%
  ...
  ungroup() %>%
  mutate(expertAnno.l1 = fct_reorder(expertAnno.l1,dplyr::desc(med)))
...
ggsave("../../out/image/20_FindMarkers_cellID_treat_minpct5_logfcthr02_refBASELINE.pdf",width = 17,height = 15)
